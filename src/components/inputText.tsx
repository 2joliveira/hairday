...
interface InputTextProps
  extends React.ComponentProps<"input">, VariantProps<typeof inputTextVariants>
{
  icon?: React.ComponentProps<typeof Icon>["svg"];
}

export function InputText({ className, variant, icon, ...props }: InputTextProps) {
  return (
    <div className={inputTextVariants({ className, variant })}>
      {icon && <Icon svg={icon} className={iconVariants({ size: "md" })} />}
      <input type="text" {...props} className={cx(textVariants(), "outline-none w-full h-full")} />
    </div>
  )
}